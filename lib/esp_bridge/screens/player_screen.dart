...
class _PlayerScreenState extends State<PlayerScreen>
      ...
      appBar: AppBar(
        ...
        leading: widget.showMenuButton
            ? IconButton(
                ...
                icon: Icon(Icons.menu_rounded, color: t.textMuted),
              )
            : null,
        ...
        builder: (context, snapshot) {
          ...
          return Padding(
            ...
            child: Column(
              ...
              children: [
                _buildMetadataDisplay(info, t),
                const SizedBox(height: 40),
                _buildProgressDisplay(info, t),
                const SizedBox(height: 40),
                _buildControls(info, t),
              ],
            ),
          );
        },
  ...
  Widget _buildConnectionBadge(GanciTheme t) {
  ...
  Widget _buildMetadataDisplay(MediaInfo? info, GanciTheme t) {
    Widget albumArt;
    if (info != null && info.thumbnailUrl.isNotEmpty) {
      ...
      if (_cachedImage != null) {
        albumArt = ClipRRect(
          ...
          child: Image.memory(_cachedImage!, width: 240, height: 240, fit: BoxFit.cover, gaplessPlayback: true,
            errorBuilder: (_, __, ___) => Icon(Icons.music_note_rounded, size: 80, color: t.primaryLight.withOpacity(0.4)),
          ),
        );
      } else if (Uri.tryParse(info.thumbnailUrl)?.hasScheme == true) {
        albumArt = ClipRRect(
          borderRadius: BorderRadius.circular(30),
          child: Image.network(info.thumbnailUrl, width: 240, height: 240, fit: BoxFit.cover,
            errorBuilder: (_, __, ___) => Icon(Icons.music_note_rounded, size: 80, color: t.primaryLight.withOpacity(0.4)),
          ),
        );
      } else {
        albumArt = Icon(Icons.music_note_rounded, size: 80, color: t.primaryLight.withOpacity(0.4));
      }
    } else {
      _lastThumbnail = null;
      _cachedImage = null;
    ...
    return Column(
      children: [
        ...
        Text(
          ...
          style: TextStyle(color: t.textPrimary, fontSize: 24, fontWeight: FontWeight.bold, fontFamily: 'Inter'),
          textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis,
        ),
        const SizedBox(height: 8),
        Text(
          info?.artist ?? 'Unknown Artist',
          style: TextStyle(color: t.textMuted, fontSize: 16, fontFamily: 'Inter'),
  ...
  Widget _buildProgressDisplay(MediaInfo? info, GanciTheme t) {
    ...
    if (_isDragging) {
      ...
      progress = (info != null && duration > 0) ? (info.position / duration).clamp(0.0, 1.0) : 0.0;
      ...
      children: [
        SliderTheme(
          data: SliderTheme.of(context).copyWith(
            ...
            thumbColor: t.primaryLight,
          ),
          child: Slider(
            value: progress,
            onChangeStart: (v) {
              setState(() { _isDragging = true; _dragProgress = v; });
              _dragSendTimer?.cancel();
              _dragSendTimer = Timer.periodic(const Duration(milliseconds: 50), (_) {
        ...
        Padding(
          ...
          child: Row(
            ...
            children: [
              Text(formatTime(displayPosition), style: TextStyle(color: t.textMuted, fontSize: 12, fontFamily: 'Inter')),
              Text(formatTime(duration), style: TextStyle(color: t.textMuted, fontSize: 12, fontFamily: 'Inter')),
            ],
          ),
        ),
      ],
  ...
  Widget _buildControls(MediaInfo? info, GanciTheme t) {
      ...
      children: [
        ...
        GestureDetector(
          ...
          child: Container(
            width: 72, height: 72,
            decoration: BoxDecoration(
              gradient: LinearGradient(colors: [t.primary, t.primaryContainer]),
  ...
  Widget _buildPermissionPrompt(GanciTheme t) {
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Icon(Icons.lock_person_rounded, color: t.primary, size: 64),
            const SizedBox(height: 24),
            Text('Akses Diperlukan', style: TextStyle(color: t.textPrimary, fontSize: 20, fontWeight: FontWeight.bold, fontFamily: 'Inter')),
            const SizedBox(height: 12),
            Text(
              'Aplikasi butuh izin "Notification Listener" untuk mengirim data lagu ke ESP32.',
              textAlign: TextAlign.center,
              style: TextStyle(color: t.textMuted, fontSize: 14, fontFamily: 'Inter'),
            ),
            const SizedBox(height: 32),
            ElevatedButton(
              onPressed: () async {
